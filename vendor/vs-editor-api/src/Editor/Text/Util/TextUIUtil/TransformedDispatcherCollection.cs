...
{
    internal class TransformedDispatcherCollection<TSourceCollection, TSourceElement, TTargetElement> : ReadOnlyCollection<TTargetElement>, INotifyCollectionChanged, INotifyPropertyChanged, IDisposable
        where TSourceCollection : class, IEnumerable<TSourceElement>, INotifyCollectionChanged
    {
            ...
            base(new List<TTargetElement>(sourceCollection.Select(setup)))
        {
            ...
            // Morgania: WPF's CollectionChangedEventManager (weak events) has no Avalonia
            // equivalent; a direct subscription is used and removed in Dispose. Consumers
            // must dispose this collection to unhook from a longer-lived source.
            this.sourceCollection.CollectionChanged += this.OnSourceCollectionChanged;
        }
        #endregion
